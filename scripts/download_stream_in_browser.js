...
(async () => {
  console.log("=== PLAYWRIGHT IN-BROWSER FETCH & ENCODING ===");
  ...
  for (const item of VALID_LINKS) {
    ...
    if (streamUrl && streamUrl.startsWith('http')) {
      ...
      console.log(`   Downloading video stream inside browser to source_${item.id}.mp4...`);

      try {
        const base64Data = await page.evaluate(async (sUrl) => {
        ...
        fs.writeFileSync(sourceMp4, Buffer.from(base64Data, 'base64'));
        console.log(`   DOWNLOADED: source_${item.id}.mp4 (${(fs.statSync(sourceMp4).size / 1024 / 1024).toFixed(2)} MB)`);
